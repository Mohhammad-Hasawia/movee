...
export const AdminUsers: React.FC = () => {
  ...
  const renderUserTable = (users: (User | Driver)[], isDriver: boolean = false, showOfficeAction: boolean = false) => {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {(isDriver || !isDriver) && <TableHead>{t.rating}</TableHead>}
        ...
        <TableBody>
          {filteredUsers.length === 0 ? (
            <TableRow>
              <TableCell colSpan={6 + (isDriver ? 2 : 0) + (showOfficeAction ? 1 : 0)} className="text-center py-8 text-muted-foreground">
                {t.noUsers}
              </TableCell>
            </TableRow>
          ) : (
            filteredUsers.map((user) => (
              <TableRow key={user.id}>
                ...
                {showOfficeAction && (
                  <TableCell>{user.office ? `Office ${user.office}` : '—'}</TableCell>
                )}
                <TableCell>

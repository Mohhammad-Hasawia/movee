...
import { AlertTriangle, Loader2, RefreshCw, CheckCircle2, UserCircle, CalendarDays, Star, FileText } from 'lucide-react';
                      ...
                      <TableRow key={alert.id} className="cursor-pointer hover:bg-muted/30" onClick={() => setSelectedAlert(alert)}>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">
                              {getAlertDisplayValue(alert, ['employee_name', 'worker_name', 'driver_name', 'name']) as string}
                            </span>
                          </div>
                        </TableCell>
                        <TableCell>
                          {getAlertDisplayValue(alert, ['account_type', 'employee_type', 'type']) as string}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            {String(getAlertDisplayValue(alert, ['current_rating', 'rating']) || '—')}
                          </div>
                        </TableCell>
                        <TableCell>
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-lg border p-4 space-y-2">
                  ...
                  <p className="font-medium">{getAlertDisplayValue(selectedAlert, ['employee_name', 'worker_name', 'driver_name', 'name']) as string}</p>
                </div>
                <div className="rounded-lg border p-4 space-y-2">
                  <p className="text-sm font-semibold text-muted-foreground">{t.columns.type}</p>
                  <p className="font-medium">{getAlertDisplayValue(selectedAlert, ['account_type', 'employee_type', 'type']) as string}</p>
                </div>
                <div className="rounded-lg border p-4 space-y-2">
                  <p className="text-sm font-semibold text-muted-foreground">{t.columns.rating}</p>
                  <p className="font-medium">{getAlertDisplayValue(selectedAlert, ['current_rating', 'rating']) as string}</p>
                </div>
                <div className="rounded-lg border p-4 space-y-2">
